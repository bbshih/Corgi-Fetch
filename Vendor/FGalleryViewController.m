...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
	if((self = [super initWithNibName:nil bundle:nil])) {
        ...
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
            size = kThumbnailIPadSize;
            spaceSize = kThumbnailIPadSpacing;
        } else {
            size = kThumbnailSize;
            spaceSize = kThumbnailSpacing;
        }
        [self setThumbnailSize:size];
        [self setThumbnailsSpacingSize:spaceSize];
...
- (void)loadView
{
    ...
    _caption.font								= [UIFont fontWithName:@"HelveticaNeue-Medium" size:14.0f];
...
- (void)reloadGallery
{
    ...
    if ([_photoSource numberOfPhotosForPhotoGallery:self] > 0) {
        ...
        // layout
        [self layoutViews];
        if (_thumbsView.contentOffset.y != 4.0) {
            [_thumbsView setContentOffset:CGPointMake(_thumbsView.contentOffset.x, -kThumbnailSpacing) animated:NO];
        }
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (_beginsInThumbnailView && _useThumbnailView && _currentIndex == 0) {
...
//    
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Sort", @"") style:UIBarButtonItemStylePlain target:self action:@selector(showSortMenu:)] autorelease];
...
- (void)enterFullscreen
{
	if (!_isThumbViewShowing && ![self hasMovedToOtherView])
    {
        [self disableApp];
        _isFullscreen = YES;
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
		[[UIApplication sharedApplication] setStatusBarHidden:NO animated:YES]; // 2.0 - 3.2
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
	}
...
- (void)arrangeThumbs // I override this in PhotoFGalleryViewController
{
	float dx = 0.0;
	float dy = 0.0;
...
//		[thumbView setBackgroundColor:[UIColor grayColor]];
		...
		if( dx + kThumbnailSize + kThumbnailSpacing > _thumbsView.frame.size.width )
		{
			dx = 0.0;
        ...
        [_thumbsView setContentSize:CGSizeMake( _thumbsView.frame.size.width - ( kThumbnailSpacing*2 ), dy + (kThumbnailSize + kThumbnailSpacing)*2)];
